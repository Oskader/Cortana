...
class TelegramUI:
    ...
    async def _cmd_status(
        ...
        msg = (
            ...
            f" (${bot_state.daily_pnl:+,.2f})</code>\n"
            f"Drawdown: <code>{bot_state.max_drawdown:.2%}</code>\n"
            f"Régimen: <code>{bot_state.market_regime}</code>\n"
            f"Posiciones: <code>{bot_state.open_position_count}/"
            f"{settings.MAX_OPEN_POSITIONS}</code>\n"
            f"Trades hoy: <code>{bot_state.trades_today}/"
            f"{settings.MAX_DAILY_TRADES}</code>\n"
            f"━━━━━━━━━━━━━━━━━━━━━\n"
